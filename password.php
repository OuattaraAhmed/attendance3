...
if (isset($_POST["button_action"])) {
    ...
    // $Newpass = $pass;
    $checkdata=" SELECT password FROM liste WHERE id='$id' AND password = '$pass_old'  ";
    $query=mysqli_query($conn, $checkdata);
    $row = mysqli_num_rows($query);
    if($row>0)
    {
     $update = mysqli_query($conn, "UPDATE liste SET password = '$pass_new' WHERE id='$id'");
